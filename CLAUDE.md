...
0. **`INSTRUKCE/HANDOFF-2026-05-27.md`** ← **NEJNOVĚJŠÍ HANDOFF** — Big day, ~26 commitů, ~1500 řádků. Hlavní bloky: **iCloud sync auto** (cron 30 min + onMount /contacts), **Booking masivní polish** (.ics attachment + Meet link persist + diagnose endpoint + slot kolize fix + availableFrom + publicNote + vykání + Google event bez markdown + NULL bug ve filtru listu + viditelná chyba + server log), **SMTP2GO migrace** (DKIM zatím nedotažený — todo), **/start** (tile Týden, celé jméno narozeniny, audio sekce dolů), **/ukoly + Triage** (inline editace všeho — datum/priorita/kontakt/tagy/poznámka, chip „📁 Projekt / Sekce", kontrast, šířka), **Todoist responsible_uid** (Contact.todoistUserId + collaborators endpoint), **Audio upload mobile fix** (direct file picker, mobile Safari anti-popup workaround), **AI extract žádný strop** (token limit 60k, thinkingBudget 4096, prompt přepsán — verified 29min audio: 5 → 36 úkolů), **Processing screen viditelnost** (fáze 1/2, stopky, warning >5min). 4 migrace (`20260525{20,21,22,23}0000`). TODO: DKIM v DNS pro SMTP2GO, pošta classify 49/50 errors (stejný JSON parser pattern jako u úkolů).

0aa. **`INSTRUKCE/HANDOFF-2026-05-16.md`** — Kontakty polish (7+ commitů 2026-05-16): real-time sync progress s čísly (polling à 2s, `User.contactsSyncProgress`), custom AI prompt v projektech = volný markdown `customExtract` (místo strukturovaného JSON), velký toolbar redesign (Obnovit/Zahodit/Search/Page/Uložit-iCloud/+Google/Nový/Skupina), Google status v hero, delete-everywhere v každém řádku (DB+iCloud+Google), Nuclear reset endpoint, `&#13;` posílený cleanup, filter Klienti přes groups. Migrace `20260516060000_contacts_sync_progress`. **Žádné nové cron úlohy.**

0a. **`INSTRUKCE/CONTACTS-IMPLEMENTATION.md`** ← Kompletní reference Kontakty modul **VŠECH 8 FÁZÍ HOTOVÝCH** (kontakty_brief.md F1-F8). iCloud CardDAV sync (pull+push+match A) + Google Workspace obousměrný sync (last-write-wins, 3-úrovňové párování) + `/contacts/tabulka` editor + duplicity merge UI + Find&Replace + Normalizace +420 (CZ rozsahy) + Import/Export VCF/CSV (firemní mode) + auto-backup před PUT/DELETE/merge (Restore UI 80 záloh) + banner novinek z mobilu + bulk akce + skupiny CRUD + diagnostika + Nuclear reset. **Overlay model**: iCloud core fields + Rašeliniště overlay (isVip/aliases/clientTag/callLogToken/isTeam — sync se netýká). Vlastní vCard 3.0 + CardDAV bez npm deps (Apple kvirky: default XML namespace `DAV:`, `&#13;` numeric entities, UID rotation). Google scope rozšířen `contacts.readonly` → `contacts` (reauth potřeba). **22 commitů**: `175e2fd` … `97d6f93`. Zadání: `kontakty_brief.md`.
