...
| **Todoist obousměrný sync** | cron `todoist-sync` | ✅ NOVÉ 05-02 | **Pull z Todoistu zpět do DB** (každých 5 min). Todoist je zdroj pravdy: odškrtnutí/smazání → propíše se do `Task.completedAt` a `CallLog.seenAt` (VIP výpis). Nové úkoly přidané přímo v Todoist appce → vytvoří se `Task` se `source=todoist_pull`. Per-user incremental přes Sync API + `User.todoistSyncToken`. Lib `src/lib/todoist-sync.ts`, endpoint `/api/cron/todoist-sync`. |
| Dopisy | `/letters` | ✅ |
