...
| **Týden** | `/calendar/tyden` (redirect) / `/calendar/tyden/<datum>` | ✅ NOVÉ 05-04 + Úkoly pod gridem 05-10 | Desktop pohled. 7 sloupců Po-Ne, časová osa 6-23h, default+custom rituály, výročí, multi-day spans (max 2 řádky + expand), now čára vlevo v gutteru, interpretační lišta, fullscreen mód `?naplno=1`, tisk. Print CSS v `global.css`. **Pod gridem (NOVÉ 05-10): sekce „Úkoly tento týden"** — plochý seznam Tasků s dueAt v okně Po-Ne, výška karty úměrná t-* tagu (30/60/120/240/480 px), sticky header s počtem + součtem hodin, sortění dueAt+priority, tint per kategorie (sky=klient, rose=high, lavender=delegace, peach=vlastní), klik = `/ukoly#task-<id>`. Read-only review (žádný drag-and-drop). Komponenta `WeekTasksList.tsx`. |
...
| **Kontakty — tabulka** | `/contacts/tabulka` | ✅ KOMPLETNĚ HOTOVO 05-15/16 (kontakty_brief.md F1-F8 + dnešní fixy) | **iCloud CardDAV sync** s real-time progress (polling à 2s, current/total/merged) + **Google Workspace sync** obousměrný (last-write-wins) + **velký toolbar** (Obnovit/Zahodit/Search/PageSize/Uložit-iCloud/+Google/Nový/Skupina) + status pily iCloud/Google v hero + delete-everywhere v každém řádku + tabulková editace single-click + duplicity merge UI (union-find) + Find&Replace + Normalizace +420 + Import/Export VCF/CSV (firemní mode) + auto-backup před PUT/DELETE/merge (80 UI / 500 DB) + banner novinek z mobilu + bulk akce + skupiny CRUD + **Nuclear reset** (smaže non-overlay, sync znovu) + **diagnostika** breakdown per source. **Custom AI prompt** v projektech: pokud nastaven → Stage 2 vrátí volný markdown `customExtract` místo strukturovaného JSON. **Overlay model**: iCloud core / Rašeliniště overlay (isVip/aliases/clientTag/callLogToken/isTeam). Vlastní vCard 3.0 + CardDAV bez npm deps (Apple kvirky řešené: default XML namespace `DAV:`, redirect na `p<DC>`, `&#13;` decoder + sanitize parser). Google scope `contacts.readonly` → `contacts` (reauth nutný). **Žádný cron** — vše manuálně z UI. 22 commitů: `175e2fd` … `97d6f93`. **Primární zdroj:** `INSTRUKCE/CONTACTS-IMPLEMENTATION.md`. |
