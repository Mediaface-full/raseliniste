...
| **Kontakty — tabulka** | `/contacts/tabulka` | ✅ NOVÉ 05-15 (kontakty_brief.md F1) | iCloud CardDAV sync (pull/push/match) + tabulková editace (single-click edit, dirty tracking, bulk save). Sloupce: jméno/příjmení/firma/telefon/telefon2/email/skupiny/narozeniny/flag(VIP/TEAM/K)/push. Chip seznam skupin (klik = filter). Validační filtry (bez tel/email/skupiny/firma/jména). Pagination 10/25/50/100/200. **Overlay model**: iCloud drží core fields, Rašeliniště drží isVip/aliases/clientTag/callLogToken/isTeam — sync je nepřepisuje. Match strategie A: phone/email exact → first match union (zachovat lokální VIPka/Things telefony), pak re-sync replace. Credentials sdílené s provider="icloud" z kalendáře. ContactGroup tabulka pro Apple skupiny (KIND:group vCardy). Vlastní vCard 3.0 parser/serializer + CardDAV klient bez npm deps. Detail v `INSTRUKCE/CONTACTS-IMPLEMENTATION.md`. |
