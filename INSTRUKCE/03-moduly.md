...
| **Pošta integrace (fáze 1+2+3+4)** | `/posta` modul + `/settings/integrations/google` karta | ✅ NOVÉ 05-12 — Email Intelligence kompletní: fáze 1 OAuth+import, fáze 2 klasifikace 7-polí, fáze 3 `/posta` SSR modul (digest+chips+resolve), **fáze 4 RAG embeddings + hybrid search** (chunking podle délky, Gemini text-embedding-004 768 dim, pgvector HNSW, ILIKE+vector combined 0.4/0.6, search mode s 6 facets, matched chunk highlight). Cron: `posta-sync` 15m, `posta-classify` 15m, `posta-embed` 5m, `posta-digest` daily 7:00. Backfill: `npm run posta:backfill -- --confirm` (resumable). Detail: `INSTRUKCE/POSTA-PHASE-4.md`. Zbývá fáze 5 (retention+encryption+Gmail push) + 6 (DetectedCommitment). |
