...
| **Kontakty — tabulka** | `/contacts/tabulka` | ✅ KOMPLETNĚ HOTOVO 05-15 (kontakty_brief.md F1-F8) | **iCloud CardDAV sync** (pull/push/match A) + **Google Workspace sync** (push s 3-úrovňovým párováním + cleanup duplicit + pull-back) + **tabulková editace** (single-click, dirty tracking, bulk save) + **duplicity merge UI** (union-find clustering, radio primárka, overlay-safe) + **Find&Replace** (sloupce + regex + case-sensitive, preview/apply) + **Normalizace +420** (CZ rozsahy mobile/landline, high/ambiguous confidence) + **Import VCF/CSV** (sniffer separator, collision detect, preview/apply) + **Export VCF/CSV** (scope vše/firma/skupina, firemní mode 7 polí) + **Auto-backup** před PUT/DELETE/merge (vCard 3.0 snapshot, 500 v DB / 80 v UI Restore) + **Banner novinek** z mobilu (createdAt > baseline + syncSource=icloud) + **Bulk akce** (delete/add-group/remove-group) + **Skupiny CRUD**. **Overlay model**: iCloud drží core, Rašeliniště drží isVip/aliases/clientTag/callLogToken/isTeam (sync nepřepisuje). Vlastní vCard 3.0 + CardDAV bez npm deps. Apple kvirky: default XML namespace `DAV:` bez prefixu, redirect na `p<DC>-contacts.icloud.com`. Google scope rozšířen `contacts.readonly` → `contacts` (reauth potřeba pro F6). 12 commitů: `175e2fd` … `02edce9`. **Primární zdroj:** `INSTRUKCE/CONTACTS-IMPLEMENTATION.md`. |
