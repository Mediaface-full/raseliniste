...
- Now čára napříč všemi sloupci s časovým badge
- 3 **rituály** automaticky vykreslené jako virtual events s tečkovaným okrajem + peach + ✨:
  - **Ranní pohled na den** — každý den 8:00, 5 min
...
- Pod mřížkou **interpretační lišta** (4-6 faktických vět):
  - "X hodin obsazeno z 119 dostupných"
  - "Středa je nejhustší den (8 událostí)"
  - "Sobota dopoledne je úplně volná"
  - "Žádný blok delší než 2 h pro hlubokou práci"
  - "Žádná událost s partnerkou tento týden"
- **PRAVIDLO**: lišta NEHODNOTÍ. Žádné "tento týden je přepracovaný". Pouze fakta.
- Tlačítko **"Naplno"** otevře v nové záložce s `?naplno=1` → Base layout místo Shell (rituální prostor pro nedělní pohled)

#### Měsíc (`/calendar/mesic` / `/calendar/mesic/<YYYY-MM>`) — desktop
**ORIENTAČNÍ, ne plánovací.** Petr nepotřebuje klasický Google Calendar styl s textem v buňkách (přetěžuje ho).

- 7×6 mřížka, **jen čísla dnů, ŽÁDNÉ texty events**
- **Hustota podbarvení**: 0 / 1-2 / 3-4 / 5+ events (čím tmavší, tím hustší)
- **Velké eventy** (allDay nebo >4h) = drobné tečky v rohu, barva = source
- Aktuální den: terakotový rámeček
- Aktuální týden: subtle inset
- Klik na buňku → /day/<datum>
- Pod mřížkou interpretace (3-4 věty) + legenda hustoty
- Tlačítko "Naplno" stejné jako u týdne
...
### 3.9 Výročí (`/vyroci`)
- Manuální seznam výročí (svatba, úmrtí, atd.)
- Banner na dashboardu + /start (ROSE banner) když je dnes
...
| **Now čára** | Terakotová horizontální čára aktuálního času v kalendáři |
| **Rituál** | 3 pravidelné self-care události v týdenním pohledu (ranní/páteční/nedělní) |
